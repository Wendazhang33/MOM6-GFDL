...
module MOM_CoriolisAdv
  ...
  subroutine CorAdCalc(u, v, h, uh, vh, CAu, CAv, OBC, AD, G, GV, US, CS, pbv, Waves)
    ...
    real :: u_q1, u_q2, u_q3, u_q4, u_q5, u_q6 ! is the v velocity at q points
    real :: v_q1, v_q2, v_q3, v_q4, v_q5, v_q6 ! is the u velocity at q points
    ...
    do k=1,nz
      ...
      elseif (CS%Coriolis_Scheme == wenovi_ENSTRO) then
        do j=js,je ; do I=Isq,Ieq    
          ...
          v_q1 = (v(i+1,J-3,k) + v(i,J-3,k)) * 0.5
          v_q2 = (v(i+1,J-2,k) + v(i,J-2,k)) * 0.5
          v_q3 = (v(i+1,J-1,k) + v(i,J-1,k)) * 0.5
          v_q4 = (v(i+1,J  ,k) + v(i,J  ,k)) * 0.5
          v_q5 = (v(i+1,J+1,k) + v(i,J+1,k)) * 0.5
          v_q6 = (v(i+1,J+2,k) + v(i,J+2,k)) * 0.5

          ! compute the masking to make sure that inland values are not used
          if ((G%mask2dCv(i+1,J-3) * G%mask2dCv(i+1,J-2) * G%mask2dCv(i+1,J-1) * G%mask2dCv(i+1,J) * G%mask2dCv(i+1,J+1) * G%mask2dCv(i+1,J+2) & 
               G%mask2dCv(i,J-3)   * G%mask2dCv(i,J-2)   * G%mask2dCv(i,J-1)   * G%mask2dCv(i,J)   * G%mask2dCv(i,J+1)   * G%mask2dCv(i,J+2)) == 1) then
      
              ! all values are valid, we use fifth order reconstruction
              call weno_five_reconstruction(abs_vort(I,J-3),abs_vort(I,J-2),abs_vort(I,J-1),abs_vort(I,J),abs_vort(I,J+1),abs_vort(I,J+2) & 
                                            v_q1, v_q2, v_q3, v_q4, v_q5, v_q6, v_u, q_u, .true.)

          elseif ((G%mask2dCv(i+1,J-2) * G%mask2dCv(i+1,J-1) * G%mask2dCv(i+1,J) * G%mask2dCv(i+1,J+1) & 
                   G%mask2dCv(i,J-2)   * G%mask2dCv(i,J-1)   * G%mask2dCv(i,J)   * G%mask2dCv(i,J+1)  ) == 1) then
              ...
              call weno_three_reconstruction(abs_vort(I,J-2),abs_vort(I,J-1),abs_vort(I,J),abs_vort(I,J+1) & 
                                             v_q2, v_q3, v_q4, v_q5, v_u, q_u, .true.)
      ...
      elseif (CS%Coriolis_Scheme == wenovi_ENSTRO) then
        do J=Jsq,Jeq ; do i=is,ie
          ...
          u_q1 = (u(I-3,j+1,k) + u(I-3,j,k)) * 0.5
          u_q2 = (u(I-2,j+1,k) + u(I-2,j,k)) * 0.5
          u_q3 = (u(I-1,j+1,k) + u(I-1,j,k)) * 0.5
          u_q4 = (u(I  ,j+1,k) + u(I  ,j,k)) * 0.5
          u_q5 = (u(I+1,j+1,k) + u(I+1,j,k)) * 0.5
          u_q6 = (u(I+2,j+1,k) + u(I+2,j,k)) * 0.5

          ! we compute the masking to make sure that inland values are not used
          if ((G%mask2dCu(I-3,j+1) * G%mask2dCu(I-2,j+1) * G%mask2dCu(I-1,j+1) * G%mask2dCu(I,j+1) * G%mask2dCu(I+1,j+1) * G%mask2dCu(I+2,j+1) & 
               G%mask2dCu(I-3,j)   * G%mask2dCu(I-2,j)   * G%mask2dCu(I-1,j)   * G%mask2dCu(I,j)   * G%mask2dCu(I+1,j)   * G%mask2dCu(I+2,j)) == 1) then

                  ! all values are valid, we use fifth order reconstruction
                  call weno_five_reconstruction(abs_vort(I-3,J),abs_vort(I-2,J),abs_vort(I-1,J),abs_vort(I,J),abs_vort(I+1,J),abs_vort(I+2,J) & 
                                                u_q1, u_q2, u_q3, u_q4, u_q5, u_q6, u_v, q_v, .true.)

          elseif ((G%mask2dCu(I-2,j+1) * G%mask2dCu(I-1,j+1) * G%mask2dCu(I,j+1) * G%mask2dCu(I+1,j+1) & 
                   G%mask2dCu(I-2,j)   * G%mask2dCu(I-1,j)   * G%mask2dCu(I,j)   * G%mask2dCu(I+1,j)) == 1) then

              ! only the middle values are valid, we use third order reconstruction
                  call weno_three_reconstruction(abs_vort(I-2,J),abs_vort(I-1,J),abs_vort(I,J),abs_vort(I+1,J) & 
                                                 u_q2, u_q3, u_q4, u_q5, u_v, q_v, .true.)
          else ! Upwind first order!
              if (u_v>0.) then
                  q_v = abs_vort(I-1,J)
              else
  ...
  subroutine weno_three_reconstruction(q1, q2, q3, q4, u1, u2, u3, u4, u, qr, velocity_smoothing)
    ...
    if (u>0.) then
      ...
      if (velocity_smoothing == .true.) then
        call weno_three_weight_0(u2, u3, b0)
        call weno_three_weight_1(u1, u2, b1)
      else
        call weno_three_weight_0(q2, q3, b0)
        call weno_three_weight_1(q1, q2, b1)
      endif
    else
      call weno_three_reconstruction_0(q3, q2, c0)
      call weno_three_reconstruction_1(q4, q3, c1)
      if (velocity_smoothing == .true.) then
        call weno_three_weight_0(u3, u2, b0)
        call weno_three_weight_1(u4, u3, b1)
      else
        call weno_three_weight_0(q3, q2, b0)
        call weno_three_weight_1(q4, q3, b1)
      endif
    endif
  
    tau = abs(b0-b1)
    w0  = 2./3 * (1 + (tau / (b0 + 1e-8))**2)
    w1  = 1./3 * (1 + (tau / (b1 + 1e-8))**2)
...
subroutine weno_three_reconstruction_0(q0, q1, c0)
    real, intent(in) :: q0,q1,q2
    real, intent(inout) :: w0
...
subroutine weno_three_reconstruction_1(q0, q1, c1)
    real, intent(in) :: q0,q1,q2
    real, intent(inout) :: w0
